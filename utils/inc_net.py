...
def get_backbone(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        ...
        local_weight_path = "./pretrained_weights/vit_base_patch16_224.augreg2_in21k_ft_in1k/pytorch_model.bin"
...
class BaseNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self.fc = None
        if "nb_classes" in args and args["nb_classes"] > 0:

            self.fc = self.generate_fc(self.feature_dim, args["nb_classes"]).to(self._device)
    ...
    def extract_vector(self, x):
        if self.model_type == 'cnn':
            return self.backbone(x)['features']  # ========== 修复：补充return ==========
        else:
    ...
    def forward(self, x):
        ...
        if self.fc is None:
            raise RuntimeError("self.fc Uninitialized！")
...
class IncrementalNet(BaseNet):
    ...
    def load_fc(self, task_id):
        ...
        self.fc = self.generate_fc(self.feature_dim, (task_id+1)*10).to(self._device)
        for i in range(task_id+1):
            for j in range(i,task_id+1):
                temp_weights = torch.load('CLs_weight'+str(j)+'.pt', map_location=self._device)
                temp_bias = torch.load('CLs_bias'+str(j)+'.pt', map_location=self._device)
                # the following 10 means the incremental classes
                if j == i:
                    self.fc.weight.data[i*10: i*10+10] = temp_weights.data[i*10: i*10+10] 
                    self.fc.bias.data[i*10: i*10+10] = temp_bias.data[i*10:i*10+10]
                else:
                    self.fc.weight.data[i*10: i*10+10] += temp_weights.data[i*10: i*10+10]
                    self.fc.bias.data[i*10: i*10+10] += temp_bias.data[i*10:i*10+10]
    ...
    def update_fc(self, nb_classes):
        fc = self.generate_fc(self.feature_dim, nb_classes).to(self._device)  # ========== 修改：添加to(device) ==========
        if self.fc is not None:
            ...
            if weight.shape[0] > nb_output:
                logging.warning(f"旧权重维度({weight.shape[0]})大于预期({nb_output})，自动切片前{nb_output}维")
                weight = weight[:nb_output, :]  # 只保留前nb_output行
            if bias.shape[0] > nb_output:
                bias = bias[:nb_output]
            

            if fc.weight.data[:nb_output].shape == weight.shape:
                fc.weight.data[:nb_output] = weight
                fc.bias.data[:nb_output] = bias
            else:
                logging.error(f"新fc层切片维度({fc.weight.data[:nb_output].shape})与旧权重维度({weight.shape})不匹配！")
                logging.error(f"nb_classes: {nb_classes}, nb_output: {nb_output}")

                fc.weight.data[:nb_output] = torch.zeros_like(fc.weight.data[:nb_output])
                fc.bias.data[:nb_output] = torch.zeros_like(fc.bias.data[:nb_output])
    ...
    def unset_gradcam_hook(self):
        if self._gradcam_hooks[0] is not None and self._gradcam_hooks[1] is not None:
    ...
    def set_gradcam_hook(self):
        ...
        if hasattr(self.backbone, "last_conv"):
            self._gradcam_hooks[0] = self.backbone.last_conv.register_backward_hook(backward_hook)
            self._gradcam_hooks[1] = self.backbone.last_conv.register_forward_hook(forward_hook)
        else:
            logging.warning("backbone without last_conv")
...
class DERNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.fc is None or self.aux_fc is None:
            raise RuntimeError("DERNet's fc/aux_fc Uninitialized")
    ...
    def load_checkpoint(self, args):
        ...
        model_infos = torch.load(checkpoint_name, map_location=self._device)
...
class PromptVitNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        if args["get_original_backbone"]:
            self.original_backbone = self.get_original_backbone(args).to(self._device)
        else:
...
class CodaPromptVitNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self.args = args
        self.backbone = get_backbone(args, pretrained).to(args["device"])
        self.fc = nn.Linear(768, args["nb_classes"]).to(args["device"])
        self.prompt = CodaPrompt(768, args["nb_tasks"], args["prompt_param"]).to(args["device"])
...
class MultiBranchCosineIncrementalNet(BaseNet):
    ...
    def construct_dual_branch_network(self, tuned_model):
        if 'ssf' in self.args['backbone_type']:
            ...
            self.backbones.append(get_backbone(newargs).to(self._device))
            ...
            self.backbones.append(get_backbone(self.args).to(self._device))

        self.backbones.append(tuned_model.backbone.to(self._device))
...
class FOSTERNet(nn.Module):
    ...
    def forward(self, x):
        # ========== 增加fc检查 ==========
        if self.fc is None or self.fe_fc is None:
            raise RuntimeError("FOSTERNet's fc/fe_fc Uninitialized")
    ...
    def update_fc(self, nb_classes):
        self.backbones.append(get_backbone(self.args, self.pretrained).to(self._device))
        if self.out_dim is None:
            self.out_dim = self.backbones[-1].out_dim
        fc = self.generate_fc(self.feature_dim, nb_classes).to(self._device)
        if self.fc is not None:
        ...
        new_task_size = nb_classes - sum(self.task_sizes)
        self.task_sizes.append(new_task_size)
        self.fe_fc = self.generate_fc(self.out_dim, nb_classes).to(self._device)
...
class AdaptiveNet(nn.Module):
    ...
    def forward(self, x):
        # ========== 增加fc检查 ==========
        if self.fc is None or self.aux_fc is None:
            raise RuntimeError("AdaptiveNet's' fc/aux_fc Uninitialized")
    ...
    def update_fc(self,nb_classes):
        ...
        if len(self.AdaptiveExtractors)==0:
            self.AdaptiveExtractors.append(_new_extractor.to(self._device))
        else:
            self.AdaptiveExtractors.append(_new_extractor.to(self._device))
